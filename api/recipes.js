    ...
    <div class="container">
        ...
        <div id="select-recipes" class="page active">
            ...
            <div class="action-buttons">
                <button class="btn-primary" onclick="viewCurrentMenu()">👁️ View Menu</button>
        ...
        <div id="inventory" class="page">
            ...
            <div class="action-buttons">
                <button class="btn-primary" onclick="saveInventory()">💾 Save Inventory</button>
        ...
        function displayRecipes() {
            ...
            container.innerHTML = recipes.map(recipe => `
                <div class="recipe-card ${selectedRecipes.includes(recipe.id) ? 'selected' : ''}" 
                    ...
                    <h3>${recipe.fields.Name || 'Untitled Recipe'}</h3>
                    <p><strong>Servings:</strong> ${recipe.fields.Servings || 'N/A'}</p>
                    <p><strong>Prep Time:</strong> ${recipe.fields['Prep Time (minutes)'] || 'N/A'} min</p>
                    <p><strong>Cook Time:</strong> ${recipe.fields['Cook Time (minutes)'] || 'N/A'} min</p>
                </div>
            `).join('');
        }
                ...
                <div class="menu-display">
                    ${selectedRecipeData.map(recipe => `
                        <div class="menu-item">
                            ...
                            <p><strong>Servings:</strong> ${recipe.fields.Servings || 'N/A'}</p>
                            <p><strong>Total Time:</strong> ${(parseInt(recipe.fields['Prep Time (minutes)']) || 0) + (parseInt(recipe.fields['Cook Time (minutes)']) || 0)} minutes</p>
                        </div>
                    `).join('')}
                </div>
            `;
                        ...
                        ${Object.keys(totalIngredients).map(ingName => {
                            ...
                            return `
                                <li>
                                    ...
                                    ${inInventory > 0 ? `<br><small style="color: #48bb78;">✓ You have ${inInventory} ${ing.unit} in inventory. Need ${needed} more.</small>` : ''}
                                </li>
                            `;
                        }).join('')}
                    </ul>
            ...
            try {
                ...
                container.innerHTML = menus.map(menu => `
                    <div class="saved-menu-card">
                        ...
                        <div class="action-buttons">
                            <button class="btn-primary" onclick="loadMenuToSelect('${menu.id}')">✏️ Edit</button>
                            <button class="btn-secondary" onclick="viewSavedMenu('${menu.id}')">👁️ View</button>
                            <button class="btn-danger" onclick="deleteMenu('${menu.id}')">🗑️</button>
                        </div>
                    </div>
                `).join('');
            } catch (error) {
